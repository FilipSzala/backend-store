...
public class OrderService implements IOrderService {
    ...
    private void validateQuantity(Cart cart) {
        cart.getCartItems().forEach(cartItem -> {
            ...
            int availableQuantity = productService
                    .getProductById(cartItem.getProduct().getId())
                    .getInventory();

            if (requestedQuantity > availableQuantity) {
                throw new IllegalStateException("Requested quantity for product ID "
                        + cartItem.getProduct().getId() + " exceeds available stock ("
                        + availableQuantity + ")");
            }
        });
    }
